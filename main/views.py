...
def admin_dashboard(request):
    ...
    context = {
        ...
        'total_revenue': total_revenue['total_amount__sum'] or 0,
    }
...
def admin_update_order_status(request, order_id, action):
    ...
    if action == 'confirm' and order.status == 'pending':
        ...
        DeliveryManager.assign_delivery_person(order)
        messages.success(request, 'Order confirmed.')
    elif action == 'prepare' and order.status in ['confirmed']:
        order.status = 'preparing'
        order.save()
        messages.success(request, 'Order marked preparing.')
    elif action == 'out' and order.status in ['preparing']:
        order.status = 'out_for_delivery'
        order.save()
        messages.success(request, 'Order is out for delivery.')
    elif action == 'deliver' and order.status in ['out_for_delivery']:
